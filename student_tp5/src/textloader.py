...
class TextDataset(Dataset):
    def __init__(self, text: str, *, maxsent=None, maxlen=None):
        ...
        self.phrases = [re.sub(' +', ' ', p[:maxlen]).strip() + "." for p in text.split(".") if
                        len(re.sub(' +', ' ', p[:maxlen]).strip()) > 0]
...
# Définir une fonction pad_collate_fn qui prépare un tenseur batch (de taille longueur x taille du batch où longueur
# est la longueur maximale de la séquence du batch) à partir d'une liste d'exemples du TextDataset : elle doit
# ajouter le code du symbole EOS à la fin de chaque exemple et padder les séquences avec le code du caractère nul.
def pad_collate_fn(samples: List[List[int]]):
    ...
    # On récupère la longueur maximale des séquences
    maxlen = max([len(s) for s in samples])
    # On crée un tenseur de taille maxlen+1 x taille du batch
    batch = torch.zeros((maxlen + 1, len(samples)), dtype=torch.int)  # +1 pour le token EOS
    # On remplit le tenseur avec les séquences d'entiers
    for i, s in enumerate(samples):
        batch[:len(s), i] = s.clone().detach()
        # On ajoute le token EOS
        batch[len(s), i] = EOS_IX
        # On pad avec le token BLANK
        batch[len(s)+1:, i] = PAD_IX
    return batch
...
if __name__ == "__main__":
    ...
    s_decode = " ".join(
        [code2string(s).replace(id2lettre[PAD_IX], "").replace(id2lettre[EOS_IX], "") for s in data.t()])
