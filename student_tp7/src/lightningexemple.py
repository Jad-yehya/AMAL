...
class Lit2Layer(pl.LightningModule):
    def __init__(self, dim_in, l, dim_out, learning_rate=1e-3):
        ...
        self.model = nn.Sequential(
            nn.Linear(dim_in, l), nn.ReLU(), nn.Linear(l, dim_out))
    ...
    def training_step(self, batch, batch_idx):
        ...
        self.valid_outputs.append(
            {"loss": loss, "accuracy": acc, "nb": len(x)})
        return logs
...
class LitMnistData(pl.LightningDataModule):
    ...
    def setup(self, stage=None):
        ...
        if stage == "fit" or stage is None:
            ...
            ds_train = TensorDataset(torch.tensor(ds.train.images.data(
            )).view(-1, self.dim_in).float()/255., torch.tensor(ds.train.labels.data()).long())
            train_length = int(shape[0]*self.train_ratio)
            self.mnist_train, self.mnist_val, = random_split(
                ds_train, [train_length, shape[0]-train_length])
        if stage == "test" or stage is None:
            # en phase de test
            self.mnist_test = TensorDataset(torch.tensor(ds.test.images.data(
            )).view(-1, self.dim_in).float()/255., torch.tensor(ds.test.labels.data()).long())
...
logger = TensorBoardLogger(save_dir=LOG_PATH, name=model.name,
                           version=time.asctime(), default_hp_metric=False)
