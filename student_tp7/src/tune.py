...
def objective(trial):
    from tp7 import INPUT_DIM, NUM_CLASSES, Model, run

    iterations = 200
    dims = [100, 100, 100]

    norm_type = trial.suggest_categorical(
        "normalization", ["identity", "batchnorm", "layernorm"]
    )
    normalization = norm_type

    dropouts = [
        trial.suggest_loguniform("dropout_p%d" % ix, 1e-2, 0.5)
        for ix in range(len(dims))
    ]
